...
var
  fnt_params: TFontParams;
  char_pos: Integer;
  old_start: Integer;
  old_length: Integer;
begin
  old_start:= MainRTF.SelStart;
  old_length:= MainRTF.SelLength;
  for char_pos:=old_start to old_length-1 do
  begin
    MainRTF.GetTextAttributes(char_pos,fnt_params);
    fnt_params.Size:= fnt_params.Size + 2;

    MainRTF.SetRangeParams(char_pos,1,[tmm_Size],fnt_params,[],[]);
  end;

  MainRTF.SelStart:= old_start;
  MainRTF.SelLength:= old_length;
...
begin
  ...
  AuthorEdit.Clear;
  OpenFile.Author:= '';
  MainRTF.Color:= clWhite;
  OpenFile.BackgroundColor:= '';
